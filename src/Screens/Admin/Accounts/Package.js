...
export default function Package() {
  ...
  // Adding package values
  const handlePackageAdd = (values) => {
    const config = {
      headers: {
        "x-access-token": token,
      },
    };
    // For adding the package data url
    axios
      .post(
        URL + "package",
        {
  ...
  const handlePackagedelete = (e) => {
    ...
    // Main axios part for deleting data to backend package data
    axios
      .delete(URL + "package/" + e, config)
      .then((response) => {
  ...
  const handleEdit = (e) => {
    ...
    editform.setFieldValue("validity", datas.validity);
    editform.setFieldValue("price", datas.price);
    editform.setFieldValue("strike", datas.strike);
    setOpenedEdit(true);
  };

  // For edit the package data with id
  const handlePackageEdits = (values) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        data[edit_index].validity = values.validity;
        data[edit_index].price = values.price;
        data[edit_index].strike = values.strike;
  ...
  const rows = sortedData.map((row, index) => {
    return (
      <tr key={row.id}>
        ...
        {/* Action button for edit and delete the data */}
        <td>
          <Menu
            shadow="sm"
            size="xs"
  ...
  return (
    <div>
      <BreadCrumb Text="Packages" Title="Accounts" />
      <Container size={1300}>
      ...
      <Drawer
        ...
        size={440}
      >
        {/* For adding package form data */}
        <form onSubmit={form.onSubmit((values) => handlePackageAdd(values))}>
          <TextInput
            label="Name"
            mt={5}
            required
          ...
          />
          {/* Adding selling price and strike price here */}
          <NumberInput
            label="Selling Price"
            mt={5}
            required
      ...
      <Drawer
        ...
        size={440}
      >
        {/* For editing the package data  */}
        <form
          onSubmit={editform.onSubmit((values) => handlePackageEdits(values))}
        >
          <TextInput
            label="Name"
            mt={5}
          ...
          <TextInput
            ...
            required
            placeholder="Validity"
            {...editform.getInputProps("validity")}
          />
          <TextInput
            label="Price"
            mt={5}
