...
export default function MarriageType() {
  const { classes } = useStyles();
  ...
  // Adding MarriageType values
  const handleMarriageTypeAdd = (values) => {
    const config = {
      headers: {
        "x-access-token": token,
      },
    ...
    axios
      ...
      .then((response) => {
        ...
        showNotification({
          color: "green",
          icon: <Check />,
          title: "MarriageType added successfully",
        });
        form.reset();
      })
      .catch((error) => {
        showNotification({
          color: "red",
          icon: <X />,
          title: "MarriageType didnt added ",
        });
      });
  };
  ...
  const handleModal = (e) => {
    modals.openConfirmModal({
      title: "Do you want to delete this MarriageType",
      labels: { confirm: "Confirm", cancel: "Cancel" },
      confirmProps: { color: "red" },
      onCancel: () => console.log("Cancel"),
      onConfirm: () => handleMarriageTypedelete(e),
    });
  };

  //   For delete db data from table
  const handleMarriageTypedelete = (e) => {
    ...
    // Main axios part for deleting  marriageType data
    axios
      .delete(URL + "marriageType/" + e, config)
      .then((response) => {
  ...
  // Edit function for marriageType
  const handleEdit = (e) => {
    var datas = data.find((img) => img.value === e);
    setDeleteIndex(e);
    editform.setFieldValue("marriageType", datas.label);
    setOpenedEdit(true);
  };

  const handleMarriageTypeEdits = (values) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        data[edit_index].label = values.marriageType;
        // Set loading effect animation
        updateNotification({
          id: "load-data2",
          color: "teal",
          title: "Data Updated",
          message: "MarriageType Data Updated Successfully",
          icon: <Check />,
        });
      })
      .catch((error) => {
  ...
  const form = useForm({
    ...
    validate: (values) => ({
      marriageType: values.marriageType.length < 2 ? "Too short name" : null,
    }),
  });

  // marriageType editform values
  ...
  const print = () => {
    ...
    doc.save("MarriageType.pdf");
  };
  return (
    <div>
      <BreadCrumb Text="Marriage Type" Title="Settings" titleLink="/settings" />
      <Container>
      ...
      <Drawer
        ...
        size={440}
      >
        {/* For adding marriage type data new */}
        <form
          onSubmit={form.onSubmit((values) => handleMarriageTypeAdd(values))}
        >
          <TextInput
            label="MarriageType"
            mt={5}
            ...
            placeholder="MarriageType"
            {...form.getInputProps("marriageType")}
          />
          <Button
            fullWidth
            radius="md"
      ...
      <Drawer
        ...
        title="Edit MarriageType"
        position="right"
        padding="xl"
        size={440}
      >
        {/* For edit marriage type data */}
        <form
          onSubmit={editform.onSubmit((values) =>
            handleMarriageTypeEdits(values)
          )}
        >
          <TextInput
            label="MarriageType"
            ...
            required
            placeholder="MarriageType"
            {...editform.getInputProps("marriageType")}
          />
          <Button
            fullWidth
            radius="md"
