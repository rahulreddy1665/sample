...
export default function Sect() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      // For get all the sect data list
      await axios
        .get(URL + "sect", config)
        .then((response) => {
  ...
  const handleModal = (e) => {
    modals.openConfirmModal({
      title: "Do you want to delete this Sect",
      labels: { confirm: "Confirm", cancel: "Cancel" },
      confirmProps: { color: "red" },
      onCancel: () => console.log("Cancel"),
      onConfirm: () => handleSectdelete(e),
    });
  };

  //   For delete db data from table
  const handleSectdelete = (e) => {
    ...
    // Main axios part for deleting  sect data
    axios
      .delete(URL + "sect/" + e, config)
      .then((response) => {
  ...
  const handleSectEdits = (values) => {
    ...
    // Main axios part for sending data to backend editing sect data
    axios
      .patch(
        URL + "sect",
        {
      ...
      .then((response) => {
        ...
        updateNotification({
          ...
          color: "teal",
          title: "Data Updated",
          message: "Sect Data Updated Successfully",
          icon: <Check />,
        });
      })
      .catch((error) => {
  ...
  const rows = sortedData.map((row, index) => {
    return (
      <tr key={row.id}>
        ...
        <td>{row.label}</td>
        {row.sub_cast_id !== null ? <td>{row.sub_cast.label}</td> : <td>-</td>}
  ...
  const form = useForm({
    ...
    validate: (values) => ({
      sect: values.sect.length < 2 ? "Too short name" : null,
    }),
  });

  // sect editform values
  ...
  const [headers, setHeaders] = useState([
    { label: "Sect", key: "label" },
    { label: "Sub-Cast", key: "sub_cast.label" },
  ]);

  //   geenrating pdf file for downloading the data with header defines   PDF Export
  const print = () => {
    const doc = new jsPDF();
    var body = [...data.map((el) => [el.label, el.sub_cast.label])];
    doc.autoTable({
      head: [["Sect", "Sub-Cast"]],
      body: body,
    });
  ...
  return (
    <div>
      <BreadCrumb Text="Sect" Title="Settings" titleLink="/settings" />
      <Container>
      ...
      <Drawer
        ...
        size={440}
      >
        {/* Form for adding sect data new */}
        <form onSubmit={form.onSubmit((values) => handleSectAdd(values))}>
          <TextInput
            label="Sect"
            mt={5}
            required
      ...
      <Drawer
        ...
        size={440}
      >
        {/* Form for editing the sect data */}
        <form onSubmit={editform.onSubmit((values) => handleSectEdits(values))}>
          <TextInput
            label="Sect"
            mt={5}
            required
            placeholder="Sect"
            {...editform.getInputProps("sect")}
          />
          <Select
            styles={{
              hovered: { backgroundColor: "#fdf5ef" },
