...
export default function SubCast() {
  ...
  useEffect(() => {
    // for fetching the data of sub_cast
    const fetchData = async () => {
      ...
      // For get all the subcast data list
      await axios
  ...
  // Adding SubCast values
  const handleSubCastAdd = (values) => {
    const config = {
      headers: {
        "x-access-token": token,
      },
    ...
    axios
      ...
      .then((response) => {
        ...
        showNotification({
          color: "green",
          icon: <Check />,
          title: "SubCast added successfully",
        });
        form.reset();
      })
      .catch((error) => {
  ...
  const handleModal = (e) => {
    modals.openConfirmModal({
      title: "Do you want to delete this SubCast",
      labels: { confirm: "Confirm", cancel: "Cancel" },
      confirmProps: { color: "red" },
      onCancel: () => console.log("Cancel"),
      onConfirm: () => handleSubCastdelete(e),
    });
  };

  //   For delete db data from table
  const handleSubCastdelete = (e) => {
    ...
    // Main axios part for deleting  sub_cast data
    axios
      .delete(URL + "sub_cast/" + e, config)
      .then((response) => {
  ...
  // Edit function for sub_cast
  const handleEdit = (e) => {
    var datas = data.find((img) => img.value === e);
    setDeleteIndex(e);
    editform.setFieldValue("sub_cast", datas.label);
    setOpenedEdit(true);
  };

  const handleSubCastEdits = (values) => {
    ...
    // Main axios part for sending data to backend editing sub_cast data
    axios
      .patch(
        URL + "sub_cast",
        {
      ...
      .then((response) => {
        ...
        data[edit_index].label = values.sub_cast;
        // Set loading effect animation
        updateNotification({
          id: "load-data2",
          color: "teal",
          title: "Data Updated",
          message: "SubCast Data Updated Successfully",
          icon: <Check />,
        });
      })
      .catch((error) => {
  ...
  const form = useForm({
    ...
    validate: (values) => ({
      sub_cast: values.sub_cast.length < 2 ? "Too short name" : null,
    }),
  });

  // sub_cast editform values
  ...
  const [headers, setHeaders] = useState([{ label: "SubCast", key: "label" }]);
  ...
  const print = () => {
    ...
    doc.save("SubCast.pdf");
  };
  return (
    <div>
      <BreadCrumb Text="Sub-Caste" Title="Settings" titleLink="/settings" />
      <Container>
      ...
      <Drawer
        ...
        size={440}
      >
        {/* For adding subcast data list */}
        <form onSubmit={form.onSubmit((values) => handleSubCastAdd(values))}>
          <TextInput
            label="SubCast"
            mt={5}
            required
      ...
      <Drawer
        ...
        size={440}
      >
        {/* For edit subcast data */}
        <form
          onSubmit={editform.onSubmit((values) => handleSubCastEdits(values))}
        >
          <TextInput
            label="SubCast"
            mt={5}
            required
            placeholder="SubCast"
            {...editform.getInputProps("sub_cast")}
          />
          <Button
            fullWidth
            radius="md"
