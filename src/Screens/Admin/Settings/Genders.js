...
export default function Genders() {
  ...
  // Edit function for gender
  const handleEdit = (e) => {
    var datas = data.find((img) => img.value === e);
    setDeleteIndex(e);
    editform.setFieldValue("gender", datas.label);
    setOpenedEdit(true);
  };

  const handleGenderEdits = (values) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        data[edit_index].label = values.gender;
  ...
  const form = useForm({
    ...
    validate: (values) => ({
      gender: values.gender.length < 2 ? "Too short name" : null,
    }),
  });

  // gender editform values
  ...
  const [headers, setHeaders] = useState([{ label: "Gender", key: "label" }]);
  ...
  return (
    <div>
      <BreadCrumb Text="Genders" Title="Settings" titleLink="/settings" />
      <Container>
      ...
      <Drawer
        ...
        size={440}
      >
        <form onSubmit={form.onSubmit((values) => handleGenderAdd(values))}>
          <TextInput
            label="Gender"
            mt={5}
            required
      ...
      >
        <form
          onSubmit={editform.onSubmit((values) => handleGenderEdits(values))}
        >
          <TextInput
            label="Gender"
            mt={5}
